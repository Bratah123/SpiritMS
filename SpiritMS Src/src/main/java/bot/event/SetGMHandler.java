...
public class SetGMHandler extends ListenerAdapter {
    public void onGuildMessageReceived(GuildMessageReceivedEvent event){
        String message = event.getMessage().getContentRaw();
        if(message.startsWith("!setgmlevel")){
            ...
            if(args.length <= 1){
                event.getChannel().sendMessage("Please provide the necessary arguments, !setgmlevel <character> <level>").queue();
                return;
            }
            String character = args[1];
            String gmLevel = args[2];
            byte level = Byte.parseByte(gmLevel);
            if (World.Find.findChannel(character) >= 0) {
                ...
                if(c != null){
                    c.setGmLevel(level);
                    event.getChannel().sendMessage("Successfully gave gm to " + character + ".").queue();
                    return;
                }
            }
