...
function start() {
	...
	if (cm.isGMS()) {
		...
		nitems = Array("2 million mesos", "5k Cash", "#v2340000#White Scroll x1 (level 70+)", "#v2049100#Chaos Scroll x 5 (level 70+)", "#v5062002#Super Miracle Cube x 2", "#v5050000#AP Reset x 5", "#v5062000#Miracle Cube x 4", "#v2022179#Onyx Apple x 4", "#v2022179#Onyx Apple x 10", "#v5062001#Premium Miracle Cube x 10", "#v2530000#Lucky Day (level 70+)", "#v2531000#Protection Scroll (level 70+)");
		np = Array(2, 2, 10, 6, 2, 2, 2, 2, 5, 5, 10, 15);
		np = Array(2, 2, 10, 6, 2, 2, 2, 2, 5, 5, 10, 15);
		ditems = Array("#v5211068#1.5x EXP Card (1 Day)", "#v5360000#2x Drop/Meso Card (1 Day)");
		dp = Array(5, 10, 6);
	} else {
		...
		nitems = Array("5 million mesos", "2.5k Cash", "#v2340000#White Scroll (level 70+)", "#v2049100#Chaos Scroll x 3 (level 70+)", "#v5062002#Super cube x 2", "#v5050000#AP Reset x 10", "#v5050000#AP Reset x 30", "#v5062000#Miracle Cube x 4", "#v5062000#Miracle Cube x 21", "#v2022179#Onyx Apple x 5", "#v2022179#Onyx Apple x 11", "#v5062001#Premium Miracle Cube x 2", "#v2530000#Lucky Day (level 70+)", "#v2531000#Protection Scroll (level 70+)");
		np = Array(4, 1, 10, 30, 2, 1, 2, 1, 10, 2, 10, 18, 10, 20);
		ditems = Array("#v5211077#1.5x EXP Card (1 Day)", "#v5360017#2x Drop/Meso Card (1 Day)", "#v5030006#Mushroom Elf House");
	...
	if (status == 0) {
		...
		if (cm.getPlayer().getVPoints() < np[selection]) {
			...
			if (passed) {
				...
				cm.sendOk("Thank you for the purchase~ and you have #e#r" + cm.getPlayer().getVPoints() + "#n#k Left!!!");
			}
		}
		cm.dispose();
		...
		if (cm.getPlayer().getVPoints() < 20) {
			...
			cm.sendOk("Thank you for the purchase~ #e#r" + cm.getPlayer().getVPoints() + "#n#k Left!!!");
		}
		cm.dispose();
		...
		if (cm.getPlayer().getVPoints() < 0) {
			...
			cm.sendOk("Thank you for the purchase~Your VP has not been deducted");
		}
		cm.dispose();
	}
